      ...
      <ul ref={ulRef} className='timeline timeline-snap-icon max-md:timeline-compact timeline-vertical relative'>
        {aboutMeData?.map((data, index: number) => {
          const currentFocused =
            animationY >= cumulativeHeights[index] / ulHeight && animationY < cumulativeHeights[index + 1] / ulHeight;
          ...
          return (
            <li className='min-h-52' ref={(el: any) => (boxRefs.current[index] = el)} key={data.id}>
              ...
              >
                ...
                <div className='text-lg text-primary/80'>{data.title}</div>
                <p className='font-thin whitespace-break-spaces break-keep max-w-80'>{data.content}</p>
              </div>
